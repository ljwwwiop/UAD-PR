...
def resize_text_pos_embed(posemb, length):
    ...
    posemb = posemb.reshape(1, 1, old_h, old_w)  # [1, 77, 512]
    posemb = F.interpolate(posemb, size=(length, old_w), mode='bilinear')

    return posemb.squeeze(0).squeeze(0)
...
def build_CLIP_from_openai_pretrained(name: str, image_size: Union[int, Tuple[int, int]], stride_size: int, jit: bool = False, download_root: str = None):
    ...
    # import pdb; pdb.set_trace()
    # model_path = '/opt/data/private/crossreid/Long-CLIP/checkpoints/longclip-B.pt'
    try:
        # loading JIT archive
    ...
    context_length = state_dict["positional_embedding"].shape[0] ## longclip
    # context_length = 77 ## clip
